...
module.exports = function (agenda, options) {
  options = options || {};
  agenda.on("ready", () => {
    const collection = agenda._collection?.collection || agenda?._collection || agenda.db?.collection;
    ...
    const mdb = collection?.s?.db || (agenda._mdb?.admin ? agenda._mdb : agenda._mdb.db);
